...
/* INSERTAR VIDEO EN CANVAS
    (function draw_video(){
        ctx.drawImage(video,0,0,1366,768);
        setTimeout(draw_video,10,video,ctx,1366,768);
    })();
*/
...
class App extends React.Component {
  ...
  componentDidMount() {
      ...
      Promise.all([loadlModelPromise])
        .then(values => {
          this.detectFromVideoFrame(values[0], video);
        })
        .catch(error => {
          console.error(error);
        });
  ...
  render() {
    return (
      <div>
        <canvas style={this.styles} ref={this.canvasRef} width="1366" height="768" />
      </div>
    );
  }
}
